...
class UiHelperTests(unittest.TestCase):
    def test_split_terms_handles_common_separators(self) -> None:
        self.assertEqual(split_terms("Python, SQL / AI\nData"), ["Python", "SQL", "AI", "Data"])

    def test_required_validation_catches_empty_profile(self) -> None:
        errors = validate_inputs(EDUCATION_PLACEHOLDER, "", "")
        self.assertEqual(len(errors), 3)

    def test_required_validation_catches_numeric_noise(self) -> None:
    ...
    def test_required_validation_accepts_meaningful_profile(self) -> None:
        errors = validate_inputs("Bachelor's", "Python, SQL", "Data science")
        self.assertEqual(errors, [])

    def test_confidence_band_requires_profile_alignment(self) -> None:
