...
| Model training | `career_recommender/modeling.py`, `scripts/02_train_model.py` | Builds the TF-IDF and Multinomial Naive Bayes pipeline, evaluates holdout performance, and writes artifacts. |
...
| User interface | `app.py` | Captures profile inputs and displays ranked career matches, fit score, model probability, signals, and skill recommendations. |
...
The model uses `TfidfVectorizer` for text features and `MultinomialNB` for classification. This pairing is effective for non-negative sparse text vectors and produces ranked class probabilities for the UI. The pipeline is saved as one artifact so deployment does not need to rebuild preprocessing logic at runtime. Evaluation tracks top-1 accuracy plus top-3 and top-5 match rates because the product presents ranked recommendations.
