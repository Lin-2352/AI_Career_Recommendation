...
def render_sidebar(metadata: dict, top_five_rate: float | None) -> None:
    with st.sidebar:
...
def main() -> None:
    ...
    render_sidebar(metadata, top_five_rate)
    render_header()
        ...
        with st.container(border=True):
            ...
            with st.form("career_profile_form"):
                ...
                education = st.selectbox(
                    "Education or field",
                    [EDUCATION_PLACEHOLDER, *EDUCATION_OPTIONS],
                    index=0,
    ...
    with output_col:
        with st.container(border=True):
            ...
            if submitted and validation_errors:
                ...
                recommendations = rank_profile_recommendations(artifact, profile_text, skills, interests, extra_context, top_n=5)
                best = recommendations[0]
                displayed_total = sum(item["fit_score"] for item in recommendations) or 1
                fit_share = best["fit_score"] / displayed_total
                band = confidence_band(fit_share, best["confidence"], best["profile_alignment"])
                ...
                ranking = pd.DataFrame(
                    {
                        ...
                        "Fit share": [(item["fit_score"] / displayed_total) * 100 for item in recommendations],
                        "Model probability": [item["confidence"] for item in recommendations],
                        "Profile alignment": [item["profile_alignment"] for item in recommendations],
                    }
                )
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "Profile alignment": st.column_config.NumberColumn("Profile alignment", format="%.0%"),
                    },
                )
    ...
    if metrics:
        with st.expander("Training Summary"):
            ...
            if metrics.get("source_distribution"):
                st.dataframe(source_table(metrics), hide_index=True, width="stretch")
